...
public:
	...
	void prepareInstanceData()
	{
		...
		for (auto i = 0; i < INSTANCE_COUNT; i++)
		{
			...
			instanceData[i].scale = (float)(1.0f + uniformDist(rndGenerator) * 2.0f);
			instanceData[i].texIndex = (uint32_t)(textures.colorMap.layerCount * (rand() / (double)RAND_MAX));//rnd(textures.colorMap.layerCount);
		}
