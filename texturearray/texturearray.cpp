...
public:
	...
	void loadTextureArray(std::string filename, VkFormat format)
	{
		...
		textureArray.width  = (uint32_t)(tex2DArray.dimensions().x);
		textureArray.height = (uint32_t)(tex2DArray.dimensions().y);
		layerCount = (uint32_t)tex2DArray.layers();
		...
		if (sameDims)
		{
			...
			bufferCopyRegion.imageExtent.width  = (uint32_t)tex2DArray[0].dimensions().x;
			bufferCopyRegion.imageExtent.height = (uint32_t)tex2DArray[0].dimensions().y;
		...
		else
		{
			...
			for (uint32_t layer = 0; layer < layerCount; layer++)
			{
				...
				bufferCopyRegion.imageExtent.width  = (uint32_t)tex2DArray[layer].dimensions().x;
				bufferCopyRegion.imageExtent.height = (uint32_t)tex2DArray[layer].dimensions().y;
				bufferCopyRegion.imageExtent.depth = 1;
				bufferCopyRegion.bufferOffset = offset;

				bufferCopyRegions.push_back(bufferCopyRegion);

				offset += (uint32_t)tex2DArray[layer].size();
			}
		}
...
#undef dim
	...
	void prepareUniformBuffers()
	{
		...
		uint32_t uboSize = (uint32_t)(sizeof(uboVS.matrices) + (layerCount * sizeof(UboInstanceData)));
