		...
		void loadTexture(std::string filename, VkFormat format, VulkanTexture *texture, bool forceLinear, VkImageUsageFlags imageUsageFlags)
		{
			...
			if (useStaging)
			{
				...
				for (uint32_t i = 0; i < texture->mipLevels; i++)
				{
					...
					bufferCopyRegion.imageExtent.width  = (uint32_t)tex2D[i].dimensions().x;
					bufferCopyRegion.imageExtent.height = (uint32_t)tex2D[i].dimensions().y;
					bufferCopyRegion.imageExtent.depth = 1;
					bufferCopyRegion.bufferOffset = offset;

					bufferCopyRegions.push_back(bufferCopyRegion);

					offset += (uint32_t)tex2D[i].size();
				}
